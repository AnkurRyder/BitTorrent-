<?php
	...
	function register() {
		...
		$link = $db -> quote(htmlspecialchars($_POST['link']));
				...
				if(passwords_match($password,$toValidatePassword)) {
					if(valid_password(htmlspecialchars($_POST['password']))) {
						if(!already_registered($email,$username,$db)) {
							...
							$result = $db -> query("INSERT INTO `users` (`email`, `reg_date`, `username`, `password`, `tempkey` , `profile_image`) VALUES (" . $email . "," .$mysql_date . "," . $username.",'".$hashed_password ."','".$key ."',".$link.")"); 
