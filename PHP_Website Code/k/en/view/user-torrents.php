<?php
    ...
    if(!empty($_GET["userid"]) && isset($_GET["userid"]) && is_numeric($_GET["userid"])) {
        $userid = $_GET["userid"];
        $torrents = $db -> select("SELECT t.userid,t.categoryid,t.name,t.uploaddate,t.size,t.seeders,t.leechers,t.hash,t.magnet,u.profile_image,u.username,u.uploaderstatus,u.email,c.id,c.categoryname FROM `torrents` t INNER JOIN `users` u ON t.userid=u.user_id INNER JOIN `categories` c ON t.categoryid=c.id WHERE t.userid=".$userid."");
    } else {
...
<body>
    ...
    <div class="container">
        ...
        <div class = "row">
        <div class="col-lg-12">
            <img src="<?php echo($torrents[0]["profile_image"]) ?>" class="center" alt="Smiley face" style="width:200px;height:200px; overflow: hidden;">
            <p><h3 style = "margin-top : 30px ; text-align:center">User Name : <?php echo($torrents[0]["username"]) ?> </h3>
            <h3 style = "margin-top : 20px ; text-align:center ;margin-bottom:30px;">Email Id : <?php echo($torrents[0]["email"]) ?> </h3></p>
            </div>
        </div>
        <!-- <div class="row">
            <div class="col-lg-12">
